...
(
    ...
    # OT-side per-chapter artifacts (added 2026-05-10 — Ruth/Jonah/Genesis pilot
    # introduced these files; previously they accumulated as untracked orphans
    # until the end-of-book audit subagent picked them up. This list is a
    # superset — patterns silently no-op for chapters that don't generate them.)
    git add "output/textual_variants/${SLUG}_${CHAPTER_PADDED}.json" 2>/dev/null || true
    git add "output/check_reports/honorifics_binding_${SLUG}_${CHAPTER_PADDED}.md" 2>/dev/null || true
    git add "output/check_reports/divine_names_${SLUG}_${CHAPTER_PADDED}.md" 2>/dev/null || true
    git add "output/check_reports/hebrew_field_integrity_${SLUG}_${CHAPTER_PADDED}.md" 2>/dev/null || true
    git add "output/check_reports/versification_${SLUG}_${CHAPTER_PADDED}.md" 2>/dev/null || true
    ...
    # Cumulative OT-side data (versification map + idioms ledger may grow when
    # OT chapters add divergence entries or new idiom encounters)
    git add data/versification_map.json data/hebrew_idioms.json 2>/dev/null || true
    ...
    git add output/check_reports/versification_anchor.md 2>/dev/null || true
