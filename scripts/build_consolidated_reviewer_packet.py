...
def build_english(book_codes):
    ...
    slug_examples = ", ".join(f"`{BOOKS[c][0]}` for {BOOKS[c][1]}" for c in book_codes)
...
> **A note on `<slug>` and `<book>` in file paths:** `<slug>` means the lowercase no-spaces book name — {slug_examples}. `<BOOK>` means the three-letter uppercase book code (MAT, MRK, LUK, JHN, ACT, etc.). When a path says `output/reader/<slug>.md`, you read it as `output/reader/luke.md` for Luke, and so on.
