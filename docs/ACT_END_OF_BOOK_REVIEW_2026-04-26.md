...
**Acts 26:23 is the cleanest exegetical proof of the rule** — the verse maps λαός→ชนชาติ (Israel-as-nation) against ἔθνεσιν→คนต่างชาติ (Gentiles) within a single doublet. Translator clearly distinguishes the two semantic fields.
...
### B. ἔθนος split — confirm + new doc (§1)
...
**Acts ships in strong corpus-hygiene shape.** All automated checks pass; inherited Luke locks are compliant; the three undocumented-but-stable patterns (ἔθνος / Roman titles / pagan deities) are coherent at the verse level and just need lifting to corpus docs.

**One DECIDE item** (Acts 21 we-register) blocks `book-acts-v1` tag; the rest can be handled as either pre-tag tightening or post-tag follow-ups.

**Recommend tag `book-acts-v1`** after:
1. Ben decides §5 (Acts 21 we-register).
2. Three new translator_decisions docs written (§1, §2, §3) — or deferred as documented post-tag follow-ups.
3. At least one external AI sanity-check review (§F).
4. RULES.md update if any §A–E decisions warrant canonical documentation.
