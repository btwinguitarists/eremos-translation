...
> **Disposition (2026-06-27, Ben):** 5 are word-choices decidable on the project's own criteria (source-text faithfulness + natural Thai + corpus consistency) — recommended calls: jealousy → `ริษยา` (NT-thread + the sense is *envy*, not romantic `หึง`); Matt-4 test-God → `ทดลอง` ("put to the test / tempt", vs neutral `ทดสอบ`); governor `פֶּחָה` → `ข้าหลวง` (an official appointed by a distant empire — fits the Persian satrap system); Stone → `อับอาย` (honor/shame-accurate + concordance with the other citation); goat-demon → `แพะป่า` in the desolation passages (naturalistic, matching the surrounding animals), `ผีปีศาจรูปแพะ` kept in the cult contexts.
> **Only 4 need a real decision** (to the Thai reviewer): **God's "soul" (Isa 42:1 // Matt 12:18)** · **Nebuchadnezzar register** · **Ps 82 divine-council `เทพ`** · **generic wisdom-king register**.
...
### T6 — human-king register: generic wisdom-kings (cross-book conflict)
- **Generic "king" rendered inconsistently across the wisdom books:** Proverbs treats generic kings **plain** (16:10 `…กษัตริย์…ท่าน`, 16:14 `ความโกรธของกษัตริย์` — no `ทรง`), but **Ecclesiastes royal** (8:4 `พระดำรัส…พระองค์ทรง`). Song of Songs gives the *specific* king Solomon full `ราชาศัพท์` (3:11 `ทรงมงกุฎ`) — defensible per `ot_register_policy §2.2`.
- §2.2 elevates *specific* Hebrew kings in public-office to `ราชาศัพท์` but does not address **generic wisdom-saying kings**. **Q:** unify the generic-king register (Proverbs-plain vs ECC-royal)? This is the OT-internal counterpart to the foreign-emperor register question (`JER-T6-001` Nebuchadnezzar).
- **Sub-point (`GEN-T6-001`):** Joseph-as-vizier may be wrongly given `ทรง` somewhere — §2.5 keeps patriarchs/vizier **plain** (Joseph is second to Pharaoh, not a king). Verify + fix if confirmed. _(from `PRO-T6-001`, `SNG-T6-001`, `GEN-T6-001`)_

### T8 — goat-demon `שָׂעִיר`: demon vs wild-goat (desolation passages)
- **`שָׂעִיร` rendered 2 ways:** `ผีปีศาจรูปแพะ` ("goat-demon") at Isa 13:21, Lev 17:7, 2Chr 11:15 vs `แพะป่า` ("wild goat") at Isa 34:14. The **cult** contexts (Lev 17:7, 2Chr 11:15 — worshipped idols) are consistently demonic; but the two parallel **desolation** passages split — **Isa 13:21 `ผีปีศาจรูปแพะ` vs Isa 34:14 `แพะป่า`**. **Q:** unify the desolation-`שָׂעִיר` rendering (demon vs wild-goat)? (Cult-`שָׂעִיร` is consistently demonic; Ben's exegetical call on the desolation sense — the within-Isaiah inconsistency is the flag.) _(from `ISA-T8-002`)_
