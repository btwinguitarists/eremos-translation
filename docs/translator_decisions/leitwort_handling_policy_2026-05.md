...
- Normalize the **bare `จนถึงวันนี้`** occurrences UP to `จนถึงทุกวันนี้`, verified verse-by-verse against `עַד הַיּוֹם הַזֶּה`. **NB:** this **reverses** the 2026-05-23 instruction (normalize Judges' `ทุก-` *down* — now superseded). **Caution — `עַד`/`עֵד` homograph:** a consonantal `עד` scan false-positives on `עֵד` ("witness," e.g. 1SA 12:5 "a witness this day" `พยานในวันนี้`); confirm `עַד` ("until") before changing.
  - **1 Samuel — ✓ DONE 2026-05-26** (1CH EOB Item E; both external reviewers): **9** verses normalized — 5:5, 6:18, 8:8, 12:2, 27:6, 29:3, 29:6, 29:8, 30:25. (12:5 left as `พยานในวันนี้` — `עֵד` witness, not the formula. The doc's earlier "23×" estimate was wrong; the verified count is 9.)
  - **Still pending (verified counts, 2026-05-26 corpus scan — supersede the earlier ~46 estimate):** 2 Samuel **4×**, Joshua **2×**, Judges **1×**, Deuteronomy **2×** (+ 2 "other" to inspect), Exodus **1×**, Numbers **1×**. 1 Kings is **already compliant** (0 bare; earlier "5×" was wrong). A handful of "other" surfaces (e.g. 2KI 7:9, 19:3; JOS 22:16; GEN 31:48; LEV 23:14) need per-verse Hebrew checks (homograph / non-etiological) before any change. **Not blocking any tag.**
  - Defer a machine-enforced `check_phrase_consistency.py` leitwort lock until the pending list above is cleared (it would fail on those bare verses now).
...
- **v0.6** (2026-05-26, 2 Chronicles EOB Item F) — 2 Chronicles ships **fully conformant** (8× `จนถึงทุกวันนี้`, 0 bare in verse text). The 2CH audit §14's "1 Samuel is the lone outlier (`จนถึงวันนี้`)" was **stale** — superseded by v0.5, which had already normalized 1 Samuel's verse text. Residual `จนถึงวันนี้` in 1 Samuel was **editorial metadata only** (`thai_summary` / `notes` / `key_decisions[].rendering/note/rationale`), normalized this pass; 1 Samuel is now 0-bare anywhere. Deferred verse-text backlog **unchanged** (2 Sam 4× + Josh/Deut/Gen/Exod/Judg/Num + Acts 1× — per-verse Hebrew/Greek check still required; not a tag blocker).
- **v0.5** (2026-05-26, 1 Chronicles EOB Item E) — §7.5 back-propagation executed for **1 Samuel** (9 verses → `จนถึงทุกวันนี้`; both external reviewers). Stale deferred-ticket counts corrected against a verified corpus scan (1 Samuel = 9, not 23; 1 Kings already compliant, not 5×). Added the `עַד`/`עֵד` homograph caution (1SA 12:5 stays `พยานในวันนี้`). 2 Sam (4×) + scattered Josh/Judg/Deut/Exod/Num stragglers remain the deferred queue.
